...
class BudionApiClient:
    ...
    async def create_shopping_list_item(
        ...
        *,
        ...
        data = await self._request(
            "POST",
            f"families/{family_id}/shopping-lists/{list_id}/items",
            json=payload,
            expected_status=201,
        )
        assert isinstance(data, dict)
        item = data.get("item", data.get("data", data))
    ...
    async def update_shopping_list_item(
        ...
        **fields: Any,
        ...
        data = await self._request(
            "PATCH",
            f"families/{family_id}/shopping-lists/{list_id}/items/{item_id}",
            json=fields,
        )
        assert isinstance(data, dict)
        item = data.get("item", data.get("data", data))
    ...
    async def reorder_shopping_list_items(
        self,
        family_id: int,
        list_id: int,
        item_ids: list[int],
        *,
        ...
        """Reorder shopping list items within the open or checked group."""
        data = await self._request(
            "POST",
            f"families/{family_id}/shopping-lists/{list_id}/items/reorder",
            json={"item_ids": item_ids, "checked": checked},
        )
        assert isinstance(data, dict)
        return data
